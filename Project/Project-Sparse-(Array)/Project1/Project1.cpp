...
public:
	...
	/*double_linedlist getdLinkedlist() {
	}*/
};
class SparseMatrix {//contain the double linkedlist
	int col{};
	int row{};
	double_linedlist dll;
...
public:
		...
		for (ArrayLinkedList *i = headlist; i; i = i->nextlist) {
			if (headlist ==nullptr&& taillist==nullptr) {
				...
				*/
				...
				i->nextlist->set_value(val, indxc);//i->set_indxlist(row);
				getin = true;
				break;
			}
		}
	...
	ArrayLinkedList* checkexist(int indxrow) {
		for (ArrayLinkedList* i = headlist; i; i = i->nextlist) {
			if (i->get_indxlist() == indxrow)return i;
		}
		return nullptr;
	}
	void add(SparseMatrix &y) {
		for (ArrayLinkedList* j = y.headlist; j; j = j->nextlist) {//looping on the y sparseMat
			...
			else {//whole array not exist
				//taillist->nextlist=j;
				//for (ArrayLinkedList* i = j; i; i = i->nextlist) {
					for (int q = 0; q < row; q++) {
	...
	//bool AddOnVal(int val, int indxr, int indxc) {
	//	for (ArrayLinkedList* j = headlist; j; j = j->nextlist) {
	//		if (checkexist(j->get_indxlist()) != nullptr) {
	//			//j->add(val, indxc);
	//		}
	//	}
	//	return false;
...
int main() {
	...
	mat.set_value(2, 3, 2);
	mat.set_value(0, 3, 2);
	//mat.set_value(5, 1, 9);//this
	//mat.set_value(10, 3, 2);//line 92
	//mat.set_value(7, 3, 6);
	mat.set_value(6, 5, 6);
	mat.set_value(4, 5, 4);
	mat.set_value(3, 7, 3);
	...
	//mat2.set_value(3, 3, 3);
	mat2.set_value(5, 1, 9);//this
	//mat2.set_value(2, 3, 2);
	//mat2.set_value(5, 7, 1);
	mat2.set_value(6, 3, 8);
	mat2.set_value(9, 9, 9); //this
	//mat2.set_value(5, 3, 5); //this
	//cout << "===\n";
	mat2.print_nonzero();
	mat2.print();
	cout << "\n===\n";
